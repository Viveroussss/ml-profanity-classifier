...
if page == "📊 Data Loading":
    ...
    st.markdown("Upload your datasets or use the existing ones in the `content/` directory.")
    ...
    with col1:
        ...
        train_option = st.radio(
            ...
            ["Use existing file (content/train.csv.zip)", "Upload new file"]
        )
        ...
        if train_option == "Upload new file":
            ...
            if st.button("Load from content/train.csv.zip"):
                try:
                    if os.path.exists('content/train.csv.zip'):
                        ...
                        st.error("File not found: content/train.csv.zip")
                except Exception as e:
    ...
    with col2:
        ...
        test_option = st.radio(
            "Test data source",
            ["Use existing files (content/raw_test.csv & test_labels.csv)", "Upload new files"]
        )
        ...
        if test_option == "Upload new files":
            ...
            if st.button("Load from content/"):
                try:
                    if os.path.exists('content/raw_test.csv') and os.path.exists('content/test_labels.csv'):
                        ...
                        st.error("Files not found in content/ directory")
                except Exception as e:
    ...
    if st.button("🚀 Start Training", type="primary"):
        with st.spinner("Training model... This may take a few minutes."):
            try:
                ...
                joblib.dump(st.session_state.vectorizer, 'content/saved_models/tfidf_vectorizer.pkl')
                joblib.dump(st.session_state.model, 'content/saved_models/linear_svc_model.pkl')
...
# Page 3: Prediction
    ...
    if not st.session_state.model_trained:
        ...
        try:
            if os.path.exists('content/saved_models/tfidf_vectorizer.pkl') and \
               os.path.exists('content/saved_models/linear_svc_model.pkl'):
                st.session_state.vectorizer = joblib.load('content/saved_models/tfidf_vectorizer.pkl')
                st.session_state.model = joblib.load('content/saved_models/linear_svc_model.pkl')
                st.session_state.model_trained = True
